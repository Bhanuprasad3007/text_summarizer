...
# Ensure Transformers does not attempt to use TensorFlow/Flax
...
# Cache for summarization pipelines keyed by model name
_SUMMARIZERS = {}

# 3. Abstractive Summarization
def abstractive_summarizer(text, model_name='sshleifer/distilbart-cnn-12-6', max_chunk_size=300):
    """Summarizes text using a pre-trained transformer model (cached)."""
    summarizer = _SUMMARIZERS.get(model_name)
    if summarizer is None:
        summarizer = pipeline("summarization", model=model_name, framework="pt", device=-1)  # Force PyTorch on CPU
...
def main():
    ...
    if not os.path.exists(pdf_file_path):
        ...
        if candidates:
            pdf_file_path = candidates[0]
            print(f"Using detected PDF: {pdf_file_path}")
        else:
            print("No PDF file found in the current directory. Please place a PDF here or update 'pdf_file_path'.")
            return

    # Step 1: Extract and Clean Text
    raw_text = extract_text_from_pdf(pdf_file_path)
    if not raw_text or not raw_text.strip():
        print("The extracted text is empty. Check the PDF content.")
        return
